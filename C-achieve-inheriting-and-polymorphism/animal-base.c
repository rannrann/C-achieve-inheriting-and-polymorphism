...
animal_t * animal_init(char *name)
{
	...
	memcpy(animal, name, name_len);//void *memcpy(void *dest, const void *src, size_t n);从src的开始位置拷贝n个字节的数据到dest
...
{
	animal->animal_ops->eat(food);//指向animal_t对象的属性animal_ops，这一步很妙，封装。
	return;
}
